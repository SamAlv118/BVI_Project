...
def TargToFreqIndex(ar):
    if stype == "Absorption":
        ...
        for i in range(len(ar)):
            if ar[i] == 0:
                targIn.append(i * int((samplingrate * duration)/width))    #multiply by (551) because each pixel gets exactly (551) samples in the audio file, thats 44100*5/400 -> samplingrate * duration / pixels
    if stype == "Emission":
        targIn = []
        for i in range(len(ar)):
            if ar[i] != 0:               
                targIn.append(i * int((samplingrate * duration)/width))
    return targIn
