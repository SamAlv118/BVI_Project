...
WIDTH = 400
SAMPLING_RATE = 44100 #default sampling rate for computer audio
...
def CreateWav(stype, targs, duration):
    ...
    def TargToFreqIndex(ar):
        if stype == "Absorption":
            ...
            for i in range(len(ar)):
                if ar[i] == 0:
                    targIn.append(i * int((SAMPLING_RATE * duration)/WIDTH))    #multiply by (551) because each pixel gets exactly (551) samples in the audio file, thats 44100*5/400 -> samplingrate * duration / pixels
        if stype == "Emission":
            targIn = []
            for i in range(len(ar)):
                if ar[i] != 0:               
                    targIn.append(i * int((SAMPLING_RATE * duration)/WIDTH))
        return targIn


    #this line is going to give us the locations of each target wavelength as produced in the spectrum creator
    itargs = TargToFreqIndex(wavelengths)
    ...
    #formula: (lap time) * (total pixels in spectrum -> 400) / (set duration) + 300 = wavelength in nm
    #---------------------------------------------------------------
    ...
    with wave.open('Spectrum_to_audio.wav', 'wb') as f:
        f.setparams((2, 2, SAMPLING_RATE, 0, 'NONE', 'not compressed')) # default is (nchannels=2, sampwidth=2, framerate=44100, nframes=0, comptype='NONE', compname='not compressed')
